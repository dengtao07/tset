            ...
            >
                ...
                <el-table-column prop="audProgess" label="业务进度" align="center">
                    ...
                    <template slot-scope="scope">
                        ...
                        <el-tag  v-if="scope.row.audState == '01' && scope.row.isGiveUp == '0'">接件中</el-tag>
                        <el-tag  v-if="scope.row.audState == '02' && scope.row.isGiveUp == '0'">已接件</el-tag>
...
<script>
    export default {
        ...
        methods: {
            ...
            async getBusList() {
            //获得不同池子的所有业务信息的函数
                // console.log("我在执行getBusList函数开始。。。。。。")
            ...
            async goBackBus(row,index) {
                row.disabled = true;
                const { data: res } = await this.$http.put('v2/goBackBus',row);
                if (res.code === 200) {
                    let flag = res.data;
                    // console.log("返回的数据为：" + res.data)
                    if(flag){
